...
export type VNodeRenderer = typeof renderVNodes

export function useVueRenderer () {
  return withCtx(renderVNodes, getCurrentInstance()) as VNodeRenderer
}
...
async function renderVNodes (vNodes: Nodes | (() => Nodes | Promise<Nodes>)) {
  if (!vNodes) return ''
